...
under4 <- mean(filter(df,
                      displ <= 4)$hwy)
higher5 <- mean(filter(df,
                      displ >=5)$hwy)
c("under4" = under4,"higher5" = higher5)
...
filter(df,
       manufacturer == "audi"| manufacturer == "toyota")%>%
...
each<- filter(df,
       manufacturer == "chevrolet"| manufacturer =="ford" | manufacturer =="honda")%>%
      group_by(manufacturer)%>%
      summarise(mean(hwy))
mean(each$`mean(hwy)`)
...
top_3<-df %>% group_by(manufacturer)%>%
